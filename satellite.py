...
root_directory = 'C:\Semantic segmentation dataset'
...
model.save('models/final_model_3.hdf5')
############################################################
...
model = load_model("models/final_model_3.hdf5",compile=False)
...
test_image = image.load_img("C:/Semantic segmentation dataset/Screenshot 2023-02-16 092926.png", target_size = (256, 256)) 
...
import numpy as np
from keras.models import load_model
model = load_model("models/final_model_2.hdf5",compile=False)

app=Flask(__name__,template_folder='C:/Semantic segmentation dataset/templates',static_folder='C:/Semantic segmentation dataset/static')
...
def upload():
    ...
    #plt.savefig('C:/Semantic segmentation dataset/static/books', dpi=200)
    new_graph_name = "graph" + str(time.time()) + ".png"

    for filename in os.listdir('C:/Semantic segmentation dataset/static/'):
        if filename.startswith('graph_'):  # not to remove other images
            os.remove('C:/Semantic segmentation dataset/static/' + filename)
    
    plt.savefig('C:/Semantic segmentation dataset/static/' + new_graph_name)
